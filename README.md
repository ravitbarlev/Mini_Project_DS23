...
2. **הורד את הנתונים:** לחץ על הקישור הבא והורד את תיקיית ה-`data` המלאה מהגוגל דרייב:  
  https://drive.google.com/drive/folders/1426T_n3An72gi1EWMsGM-mJx5qhVvgMG?usp=sharing
4. **מיקום הקבצים:** ודא שתיקיית ה-`data` שחולצה נמצאת באותו המקום (באותה תיקיית אב) שבה שמרת את קובץ ה-`.py`.
5. **התקנת ספריות:** הרץ בטרמינל את הפקודה: `pip install -r requirements.txt`
6. **הרצה:** כעת ניתן להריץ את קובץ ה-`.py` והקוד יקרא את הנתונים בצורה חלקה!
